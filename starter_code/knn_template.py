...
def datingClassTest():
    ...
    hoRatio=0.50
    dating_Data_Mat,datingLabels=file2matrix(file_path)
    norMat,ranges,minVals=autoNorm(dating_Data_Mat)
    m=norMat.shape[0]
    numTestVecs=int(m*hoRatio)
    errorcount=0.0
#datingClassTest() 
    for i in range(numTestVecs):
       classifierResult = clsaaify0(normMat[i,:],normMat[numTestVecs:m,:],datingLabels[numTestVecs:m],3)
       print(f'分类器的预测结果：{classifierResult},真实结果：{datingLabelspi}')
...
def classify0(inX, dataSet, labels, k):
   ...
   sqDistances=sqDistances**5

   sortedFDistIndicies=distances.argsort()
   clsaaCount={}
for i in range(numTestVecs):
...
def classify_person():
    try:
        ...
        percent_tats=float(input("每年飞行公里数："))
